...
flowchart TD
    ...
    subgraph STAGE_2 ["2. Supply & Feasibility Review"]
        ...
        S2 -- Yes --> S3["MILP Sourcing Optimizer: MOQs, Lead Times & Risk"]
        S3 --> S4["Automated Purchase Order Release Schedule"]
    ...
    subgraph STAGE_3 ["3. In-Season Execution & Logistics"]
        ...
        L2 --> M1["POS Sell-Through Monitoring: Weeks 1 to 6"]
        ...
        M2 -- "Slow Mover (WOS > 8.0)" --> M5["Dynamic Markdown Clearance: 15% to 50% Discount"]
    end
    ...
    subgraph STAGE_5 ["5. Executive Review & Governance Lock"]
        ...
        E2 --> E3["Formal S&OP Plan Committed to ERP / MES"]
    end
```
...
* **Input Parameters**:
  - Statistical time-series forecast per SKU, region, and week (50 SKUs across 4 Regions and 12 Weeks).
  - Full-price MSRP per SKU ($29.50 to $149.00 USD).
* **Decision Rules**:
...
$$\text{Projected Revenue} = \sum_{\text{SKU}} \left( \text{New Demand Units} \times \text{Unit Retail Price} \right)$$
...
1. **Gross Explosion Formulation**:

$$\text{Gross Requirement}_{f, t} = \sum_{s \in \text{SKUs using } f} \left( \text{Demand}_{s, t} \times \text{UsagePerUnit}_{s, f} \times (1 + \text{ScrapRate}_{s, f}) \right)$$

2. **Net Inventory Netting Formulation**:
...
3. **Inventory Coverage Ratio**:

$$\text{Coverage Pct} = \left( \frac{\text{OnHand}_f + \text{InboundReceipts}_{f, t}}{\text{Gross Requirement}_{f, t} + \text{SafetyStock}_f} \right) \times 100$$

* **Operational Outputs**:
  - 450 fabric-period balance sheets.
  - Critical fabric deficit alerts and stockout vulnerabilities.
...
$$\sum_{s} x_{s, f, t} \ge \text{Net Requirement}_{f, t} \quad \forall f, t$$

2. *Minimum Order Quantity (MOQ) Constraint*:

$$\text{MOQ}_{s, f} \cdot y_{s, f, t} \le x_{s, f, t} \le \text{MaxCapacity}_{s, f, t} \cdot y_{s, f, t} \quad \forall s, f, t$$

3. *Supplier Allocation Banding Constraint*:

$$\text{MinAllocPct}_{s, f} \cdot \sum_i x_{i, f, t} \le x_{s, f, t} \le \text{MaxAllocPct}_{s, f} \cdot \sum_i x_{i, f, t} \quad \forall s, f, t$$
...
* **Interactive Sourcing & Live MOQ Validation**:
  - Sourcing sliders calculate allocated meters in real time:

$$\text{AllocatedMeters}_s = \text{NetRequirement} \times \left(\frac{\text{SliderPct}_s}{100}\right)$$

  - If $\text{AllocatedMeters}_s > 0$ and $\text{AllocatedMeters}_s < \text{MOQ}_s$, a visual `⚠️ Sub-MOQ` warning badge is triggered.
...
$$\text{Utilization Pct} = \left( \frac{\text{Allocated Units}}{\text{Max Units Capacity}} \right) \times 100$$
...
```mermaid
flowchart TD
    A["POS Sales Feeds: Weeks 1 to 6"] --> B["Compute Mean Sell-Through Rate"]
    B --> C["Compute Weeks of Stock: On-Hand / Average Weekly Sales"]
    
    C --> D{"WOS & Sell-Through Thresholds"}
    
    D -- "Fast Mover (Sell-Through >= 75% or WOS < 4.0)" --> E["Classify: FAST MOVER"]
    E --> F["Generate Stockout Vulnerability Alert"]
    F --> G["Prioritize Warehouse Replenishment & Expediting"]
    
    D -- "Slow Mover (Sell-Through <= 45% and WOS > 8.0)" --> H["Classify: SLOW MOVER"]
...
* **Logistics KPIs Monitored**:
  ...
  - Freight Cost per Unit: $1.20 to $4.80 USD.
...
$$\text{Gross Revenue} = \sum \left( \text{Forecasted Units} \times \text{Unit MSRP} \right)$$

$$\text{Net Gross Margin} = \text{Gross Revenue} - \text{Material COGS} - \text{Logistics Freight} - \text{Markdown Erosion}$$

$$\text{Gross Margin Pct} = \left( \frac{\text{Net Gross Margin}}{\text{Gross Revenue}} \right) \times 100$$
